...
const SelectOptionCard = ({ navigation }) => {
  ...
  useEffect(() => {
    setTripDistance(
      haversineDistance(origin.location, destination.location).toFixed(1)
    );
  ...
  const sendRouteToBackend = async () => {
    try {
      ...
      const useridapi =
        usertype == 2 ? userInfo.primaryUserInfo.userid : randomUser;

      //console.log(`${API_BASE_URL}/passenger/createRideRequest`);
      // console.log("userinfo:  ", userinfo);
       console.log("carrlat:  "+ pickupLat+"  currLon: "+pickupLng );
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <View style={tw`flex-row items-center`}>
              ...
              />
              <Text style={tw`ml-2`}>{item.noPerson}</Text>
            </View>
          </TouchableOpacity>
        )}
      />
      <View style={tw`m-2`}>
        <View style={tw`m-2`}>
          ...
          <Text style={tw`text-gray-500`}>
            (If join with another You can share the Cost)
          </Text>
        </View>
